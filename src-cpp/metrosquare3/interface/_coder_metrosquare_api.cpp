...
static real_T emlrt_marshallIn(const emlrtStack *sp, const mxArray *qb_0,
                               const char_T *identifier);
                               ...
                               const char_T *identifier)
{
  ...
  thisId.fParent = nullptr;
  thisId.bParentIsCell = false;
  y = emlrt_marshallIn(sp, emlrtAlias(qb_0), &thisId);
...
void probability_PIMC_api(const mxArray *const prhs[5], const mxArray **plhs)
{
  ...
  t_known = emlrt_marshallIn(&st, emlrtAliasP(prhs[3]), "t_known");
  num_grid_points_r =
      emlrt_marshallIn(&st, emlrtAliasP(prhs[4]), "num_grid_points_r");
  // Invoke the target function
